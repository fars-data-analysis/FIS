...
def getFrequentItems(data,minSupport,maxSupport):
    singleItems = data.flatMap(lambda x: [(y,1) for y in x])
    freqItems = [x for (x,y) in singleItems.reduceByKey(lambda x,y: x+y)
        .filter(lambda c: c[1]>=minSupport and c[1]<maxSupport).sortBy(lambda x: -x[1]).collect()]
    return freqItems

def getFrequentItemsets(data,minSupport,freqItems,maxLength=None):
    ...
    itemsets = forest.flatMap(lambda (partId, bonsai): bonsai.extract(minSupport, lambda x: getPartitionId(x,numPartitions) == partId, maxLength))
...
if __name__=="__main__":
    ...
    count = data.count()
    minSupport = count * threshold
    maxSupport = count * upperThreshold
    freqItems = getFrequentItems(data, minSupport, maxSupport)

    freqItemsets = getFrequentItemsets(data, minSupport, freqItems, maxLength)
